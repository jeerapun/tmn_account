 ...
 * @author roofimon
 */
@WebServlet(name = "TransferController", urlPatterns = {"/TransferController"})
public class TransferController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        TransactionHistory newTransaction = new TransactionHistory(new Date(), amount * -1, TransactionType.TRANSFER);
        ...
        //fund in
        Account accountTransferM = accountService.getAccountByCode(accountTransfer);
        TransactionHistory newTransactionTransfer = new TransactionHistory(new Date(), amount, TransactionType.TRANSFER);
        accountTransferM.addTransactionHistory(newTransactionTransfer);
        
        logger.debug("AccountTransfer: "+account.getCode()+" Current Balance:"+account.getBalance().toString());
        accountTransferM.deposit(amount);
        logger.debug("AccountTransfer: "+account.getCode()+" Current Balance:"+account.getBalance().toString());
        
        accountService.updateAccount(accountTransferM);
